...
- **Remaining API hardening queue**: scan หลัง slice นี้เหลือกลุ่มที่ควร harden ต่อ เช่น `mnr/cedex`, `operations/shift`, `settings/photo-retention/cleanup`, `yard/audit` และ route เก่าบางส่วนเช่น `billing/demurrage`, `documents/numbering`, `edi/schedule`, `settings/storage-rates`, `uploads`
