...
| `src/middleware.ts` | ทุก `/api/*` | **🔐 JWT enforcement** — ตรวจ Bearer token ทุก API call, exempt: login, EIR — **uploads ต้อง login แล้ว** |
| `src/middleware.ts` | ทุก `/api/*` | **🔧 Header forwarding** — `NextResponse.next({ request: { headers } })` กว่า response.headers เพื่อให้ route handler อ่าน x-user-id/x-user-role/x-customer-id ได้จริง |
...
- [x] **[P0] JWT Fail-Fast** — `lib/auth.ts` + `middleware.ts` ใช้ `getJwtSecret()` — throw ทันทีถ้าไม่ตั้งค่า `JWT_SECRET` (ไม่มี fallback `cyms-default-secret` อีกต่อไป)
...
- [x] **[P1] Middleware Header Fix** — เปลี่ยนจาก `response.headers.set()` เป็น `NextResponse.next({ request: { headers } })` — portal + role-check ทำงานได้ถูกต้อง
...
> **วันที่อัพเดทล่าสุด**: 31 มีนาคม 2569  
