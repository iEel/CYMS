...
- **Business party resolver**: เพิ่ม `src/lib/businessPartyResolver.ts` เพื่อ normalize party context กลาง (`legacyCustomerId`, `bookingCustomerId`, `billToCustomerId`, owner/line/forwarder/shipper/consignee/trucking) พร้อม validation positive integer; `POST /api/mnr`, M&R invoice creation, `POST /api/edi/bookings`, และ `PUT /api/edi/bookings` ใช้ helper นี้แทน fallback ตรง `billing_customer_id || customer_id` / `booking_customer_id || customer_id`; booking update ยัง reject party id ที่ไม่ใช่ positive integer ก่อน bind SQL
- **Remaining party adoption**: `billing/invoices` ยังเป็นงานต่อเนื่องใน plan เพื่อย้าย invoice create ให้ใช้ party resolver เดียวกันทั้งระบบ และคุม invoice grant ให้เฉพาะ bill-to/customer ของ invoice
