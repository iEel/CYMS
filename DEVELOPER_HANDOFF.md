...
### อัปเดตล่าสุด: Build Hygiene Scripts (5 มิ.ย. 2569)

รอบนี้แก้จุด friction จาก `.next` lock และไฟล์ artifact ชั่วคราวที่โผล่ใน git status:

- เพิ่ม `/tmp/`, `/.next-delete-*/`, `/.next-clean-permission-probe-*/` ใน `.gitignore` เพื่อไม่ให้ Jest cache, screenshot proof, node compile cache และ artifact ชั่วคราวหลุดเข้า commit
- เพิ่ม `scripts/clean-next-build.js` สำหรับล้าง `.next` แบบ cross-platform โดย scope เฉพาะโฟลเดอร์ `.next` ใต้ repo ปัจจุบัน
- Cleaner ใช้ strategy `rename .next -> .next-delete-*` เพื่อให้ path `.next` ว่างเร็ว แล้วค่อยลบ tombstone ใน background ลดการรอไฟล์จำนวนมากบน Windows
- Cleaner ตรวจ repo permission ก่อนล้าง `.next`; ถ้า Windows user ไม่มีสิทธิ์ rename/delete directory ใน project folder จะขึ้นข้อความ `Cannot rename folders inside ...` ให้ grant Modify/Full Control ที่ repo root ก่อน
- เพิ่ม package scripts:
  - `npm run clean:next`
  - `npm run build:clean`
- ถ้า `.next` ถูก dev server/Next process lock script จะแสดงคำแนะนำให้หยุด dev server แล้ว retry แทน error เงียบ ๆ
- อัปเดต `DEPLOYMENT_GUIDE.md` ให้ใช้ `npm run build:clean` ในขั้น production build และ troubleshooting `.next` lock

Verification รอบนี้:

```bash
npm test -- --runInBand --cacheDirectory ./tmp/jest-cache --runTestsByPath src/app/api/__tests__/build-hygiene-scripts.test.ts
node --check scripts/clean-next-build.js
npm run build:clean
```

ผลล่าสุด: build hygiene tests `1 suite / 3 tests` ผ่าน, JS syntax check ผ่าน และ `npm run build:clean` ผ่านหลังให้สิทธิ์ Modify กับ repo folder

> หมายเหตุ: ยังไม่รัน `npm run clean:next` บนเครื่อง dev ที่กำลังเปิด server อยู่ เพราะคำสั่งนี้ตั้งใจลบ `.next`; ให้หยุด `npm run dev` ก่อนใช้ `npm run build:clean`
> ถ้า clean fail หลังหยุด server แล้ว ให้ตรวจ permission ของ `D:\Antigravity\container-yard-system` ไม่ใช่เฉพาะ `.next`

### อัปเดตล่าสุดก่อนหน้า: Transport Portal Smoke Seed (5 มิ.ย. 2569)
...
- **Transport policy helper**: `src/lib/transportPortalAccess.ts` resolve actor จาก `x-user-id` เท่านั้น, เช็ค role/action (`portal.trucking.view` / `portal.driver.view`) และรองรับ action scope `transport.jobs.action` / `transport.activity.view`
...
- **Transport APIs**: เพิ่ม `/api/transport/actions` และ `/api/transport/activity` ต่อจาก `/api/transport/jobs`, `/api/transport/eir`, `/api/transport/capabilities`; jobs API return เฉพาะ operational whitelist พร้อม `availableActions`, `proofCount`, `lastActivityAt`
- **Transport action rules**: `requested/pending -> confirm_job/report_issue/add_proof`, `confirmed -> mark_arrived/report_issue/add_proof`, `issue_reported -> confirm_job/mark_arrived/add_proof`, `at_gate -> report_issue/add_proof`; `released/completed/cancelled/rejected` และ `gate_transaction` เป็น actionless จาก Transport Portal
- **Transport activity/proof tables**: migration กลางเพิ่ม `TransportJobActivities` และ `TransportJobProofs` พร้อม index ตาม `job_source/job_id/created_at`; ทุก action เขียน activity log และ proof upload อ้าง URL จาก `/api/uploads`
- **Transport UI**: `/transport` มี mobile-first worklist, summary, filter, job card, action dialog, proof upload, activity drawer, และ modal ดู Driver Copy / Trucking Copy จาก sanitized backend payload; ไม่แสดงเมนูหรือข้อมูล invoice/customer inventory/billing ใน shell นี้
- **Security policy**: driver เห็น/ทำ action เฉพาะงานที่ `driver_user_id` ตรงตัวเอง; trucking coordinator เห็น/ทำ action เฉพาะ `trucking_company_id` หรือ active trucking grant; action endpoint ไม่รับ `customer_id` จาก body/query และไม่ mutate `GateTransactions` ที่เสร็จแล้ว
...
รันบน DB จริงแล้วเมื่อ 4 มิ.ย. 2569 และ log ผ่าน step `Transport job action workflow tables`
...
ผลล่าสุด: focused Transport regression `7 suites / 50 tests` ผ่าน, `npm run lint` ผ่าน, `npm run build` ผ่าน, full Jest `156 suites / 1594 tests` ผ่าน

Browser smoke:
- เปิด `http://127.0.0.1:3005/transport` ด้วย session ผู้ดูแลระบบแล้วถูก redirect กลับ Dashboard ตาม guard ที่ตั้งใจไว้
- ฐานข้อมูลหลังล้างข้อมูลมี customer users `customer_1`, `customer_2` เป็น `customer_admin` เท่านั้น ยังไม่มี `driver_user`/`trucking_coordinator` และไม่มี transport job seed จึงยัง smoke flow จริงผ่าน Browser (เห็น job card + กด action) ไม่ได้โดยไม่สร้าง/แก้ข้อมูลลูกค้า
...
- ยังไม่มี GPS, QR gate pass, driver release, หรือ Driver/Trucking Portal UI อื่นนอก worklist/action drawer
...
- **Document Activity resolver-first access**: `/api/documents/activity` ย้ายมาใช้ `src/lib/documentActivityAccess.ts` ก่อน query `DocumentLifecycle`; invoice/EIR resolve ผ่าน `entityAccessResolver` โดยตรง ส่วน receipt/credit note resolve เอกสาร lifecycle ไปยัง invoice scope ก่อน แล้วค่อยตรวจ `requireResolvedEntityYardAccess()` ทำให้ denied yard access ไม่เห็น lifecycle rows กว้าง ๆ
- **Backward compatible receipt/CN activity**: ยังรองรับ `receipt`, `tax_receipt`, `credit_note`, prefix `RCP/REC/CN/INV/EIR` และ response shape เดิม แต่ unsupported `document_type` fail closed ด้วย `400`
- **Native action dialog cleanup**: เพิ่ม `ActionInputDialog` และย้าย flow ที่เหลือจาก `window.prompt()` / `window.confirm()` ไป custom UI แล้ว ครอบคลุม Portal grant repair, Gate In/Out waive reason, Billing receive payment, payment reconciliation, continuous reprint reason, Reefer exception, และ EDI booking/amendment note
- **Gate print-return helper**: Gate In/Out ใช้ `useGatePrintReturnDraft` กลางสำหรับ save/restore/clear draft หลังเปิด `/billing/print/continuous`; logic TTL, parse error และ cleanup อยู่ที่เดียว ลดโอกาส state หายหรือ stale draft ปนงานใหม่
- **Plan artifact**: เก็บแผนรอบนี้ไว้ที่ `docs/superpowers/plans/2026-06-03-cyms-next-security-ux-hardening.md`

ไฟล์หลัก:
- `src/lib/documentActivityAccess.ts`
- `src/app/api/documents/activity/route.ts`
- `src/components/ui/ActionInputDialog.tsx`
- `src/app/(dashboard)/gate/hooks/useGatePrintReturnDraft.ts`
- `src/app/(dashboard)/gate/GateInTab.tsx`
- `src/app/(dashboard)/gate/GateOutTab.tsx`
...
npm test -- --cacheDirectory .tmp\jest --runInBand --runTestsByPath src/lib/__tests__/documentActivityAccess.test.ts src/app/api/__tests__/documents-activity-access.test.ts src/app/api/__tests__/derived-yard-access.test.ts src/app/api/__tests__/ui-native-dialog-coverage.test.ts src/app/api/__tests__/portal-access-control-ui.test.ts src/app/api/__tests__/continuous-print-ui.test.ts src/app/api/__tests__/gate-in-workstation-ui.test.ts
npx tsc --noEmit
npm run lint
npm test -- --cacheDirectory .tmp\jest --runInBand
```

ผลล่าสุด: P0 resolver tests `3 suites / 31 tests` ผ่าน, P1 dialog tests `2 suites / 11 tests` ผ่าน, P2 Gate print draft tests `2 suites / 50 tests` ผ่าน, final focused regression `7 suites / 92 tests` ผ่าน, `npx tsc --noEmit` ผ่าน, `npm run lint` ผ่าน และ full Jest `147 suites / 1520 tests` ผ่าน

### อัปเดตล่าสุดก่อนหน้า: Gate Billing Print Return Restore + Document Actions (3 มิ.ย. 2569)
...
- **Canvas element model**: เพิ่ม `DocumentTemplateCanvasElement` ใน `src/lib/documentTemplateTypes.ts` และ normalize ผ่าน `normalizeTemplateCanvasElements()` เพื่อให้ full-form chrome, static labels, data blocks, และ calibration/test marks อยู่ใน `DocumentTemplateConfig.elements`
- **Default SONIC-style template**: `src/lib/documentTemplateDefaults.ts` seed `elements` สำหรับหัวบริษัท/logo/copy label/customer box/reference/table/payment/totals/footer ตามตัวอย่างกระดาษต่อเนื่อง และยัง keep `fields`/`sections.line_items` สำหรับ editable data layer
- **Unified renderer**: `ContinuousTaxReceipt.tsx` ใช้ `TemplateCanvasReceipt.tsx` render จาก config canvas เดียวกัน; full mode แสดง `form + data` layers ส่วน overlay mode แสดงเฉพาะ `data` layer และไม่พิมพ์ calibration layer ยกเว้น test print
- **Designer parity**: `TemplateCanvas.tsx` render full-form background/static elements จาก normalized config เดียวกับ print preview และ sync geometry ของ `line_items` element เมื่อ move/resize region ใน designer
- **Backward compatibility**: template เก่าที่ไม่มี `elements` จะถูกเติม default element model ตอน normalize โดยไม่ต้อง migrate ข้อมูลทันที และยัง validate/import/export schema เดิมได้
...
ผลล่าสุด: focused Continuous Template tests `3 suites / 99 tests` ผ่าน, `tsc` ผ่าน, `eslint` ผ่าน; Browser smoke ใน in-app browser ถูก redirect ไป `/login?from=/settings` และ `/login?from=/billing/print/continuous` เพราะ session ไม่มีสิทธิ์ใช้งานในเครื่องนี้ จึงยังไม่ได้ยืนยันด้วยภาพหลัง login
...
- **Document activity resolver adoption**: `documents/activity` ย้ายเข้า resolver-first access แล้ว โดย normalize invoice/EIR/receipt/credit note ก่อน query lifecycle rows และตรวจ yard access จาก entity scope ก่อนเสมอ
...
### อัปเดตล่าสุด: Document Template Visual Designer Phase 2.1 (27 พ.ค. 2569)

รอบนี้ปิดช่องว่างของ Visual Designer สำหรับ Continuous Tax Invoice / Receipt ให้พร้อมใช้กับฟอร์มกระดาษจริงมากขึ้น โดยยังไม่ขยาย scope ไปเอกสารชนิดอื่นและยังไม่สร้างระบบเลขเอกสารใหม่:

- **Line Items Region Editor**: designer รองรับการเลือกตารางรายการ `config.sections.line_items` เป็น region แยกจาก field ปกติ; drag/resize ใช้หน่วย mm และ renderer `ContinuousTaxReceipt.tsx` ใช้ geometry/columns/max rows จาก config ชุดเดียวกัน
...
- `src/components/document-templates/LineItemsInspector.tsx`
- `src/components/document-templates/CalibrationProfilesPanel.tsx`
- `src/components/document-templates/PrintHistoryPanel.tsx`
- `src/components/document-templates/PublishDiffDialog.tsx`
...
- `src/app/api/document-templates/print-history/route.ts`
- `src/app/billing/print/continuous/page.tsx`
...
รอบนี้ harden ชั้นข้อมูลของ Customer Portal และ EIR visibility เพื่อปิดความเสี่ยง data leakage ก่อนทำ Driver/Trucking Portal UI (หมายเหตุ: ตอนนี้ถูกต่อยอดแล้วในหัวข้อ Driver / Trucking Transport Portal Action Workflow ด้านบน):
...
- **Driver/Trucking backend policy พร้อมต่อยอด**: รองรับ `access_role = trucking/driver`, `permission_scope`, และ `valid_until` ใน backend grants/reconciler; scope รอบนั้นยังไม่ทำ UI แต่ตอนนี้มี Transport Portal worklist/action workflow แล้ว
...
- Driver/trucking grants started as backend policy only; current state has Transport Portal worklist/action workflow, while broader Driver/Trucking self-service UI remains future scope.
...
│   │       ├── GateGuardrailPanel.tsx   # Duplicate seal/plate + driver/photo guardrails (Gate Pass QR hidden until future phase)
...
- เพิ่ม `src/components/gate/GateGuardrailPanel.tsx` ใน Gate-In และ Gate-Out แสดง driver/truck master completeness, photo evidence status และ preflight warnings; ปิดการแสดง Gate Pass QR ไว้จนกว่าจะเปิด phase Gate Pass จริง
...
| **Confirmation Dialogs** | ✅ **แก้แล้ว** — action confirmation และ reason/input flow ใช้ custom dialogs (`ConfirmDialog` / `ActionInputDialog`) และมี source tests กัน `window.prompt()` / `window.confirm()` ใน call sites หลัก |
| **SQL Injection** | ✅ **แก้แล้ว** — customer branch update ใช้ validated positive integer + parameterized `NOT IN` placeholders |
| **Automated Testing** | ✅ **กลับมาเขียวแล้ว** — ล่าสุด full `npm test -- --runInBand` ผ่าน 156 suites / 1594 tests; เพิ่ม global search + TOTP 2FA + trusted device binding + PromptPay QR + Gate guided workflow + Gate operational guardrails + Billing tariff simulator + AR dunning action center + Supervisor approval inbox + Portal entity access grants + Reports action center + Offline queue + component boundary + Customer Portal bundle/dispute/ETA + Yard Planning + Yard 3D Workspace Polish + Reefer Monitoring/Exception + Document Template Designer + Billing/Settings/API/EDI Attachment hardening tests + Gate print-return restore + Document Activity resolver-first access + native action dialog cleanup + Transport Portal action workflow แล้ว และมี static guard กัน runtime DDL ทั้ง `src/app/api` + `src/lib` |
