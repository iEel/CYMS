...
> **Container Yard Management System** (ระบบบริหารจัดการลานตู้คอนเทนเนอร์อัจฉริยะ)  
> ส่งมอบงาน: 23 มีนาคม 2569 | เวอร์ชัน: เฟส 1-9 + FR1-6 + NFR + Master Setup + Customer Management + Gate Auto-Allocation + EIR A5 + 2-Phase Gate-Out + File Storage + Notifications + **Tiered Billing + Printable Invoice/Receipt + Bay View + 3D Search Highlight + Gate History Search + Container Detail Modal + Search Detail Panel + Boxtech API + ISO 6346 Check Digit + Prefix Mapping + Gate-In Billing + Gate-Out Billing Fix + SSE Real-Time Operations + Billing Reports + ERP Export Fix + Hold Logic Fix + Dashboard Gate-Out + CODECO Outbound EDI + SFTP Integration + 🔐 Production Readiness (Auth Middleware + Rate Limiting + Input Validation + Audit Trail) + Dwell Days Display + Demurrage Calculator + Container Tracking Timeline + 📄 Table Pagination + 🎨 Custom ConfirmDialog + 🔒 SQL Injection Audit + 🧪 Automated Testing + 📈 Dashboard Analytics (Range Toggle 7d/30d/3m) + 💳 Credit Note + 📊 AR Aging Report** (99%)
...
| **เฟส 6** | EDI, Booking/Manifest, Seal Validation, **CSV/Excel file import**, **CODECO Outbound (EDIFACT/CSV/JSON)**, **SFTP auto-upload** | ✅ เสร็จ |
...
│   │   │   ├── yard/page.tsx     # หน้าจัดการลาน (4 tabs: ภาพรวม/ค้นหา/จัดวางตู้/ตรวจนับ) + **Dwell Days column**
...
│   │   │       ├── EDIConfiguration.tsx   # SFTP/FTP/API endpoints — **CRUD เก็บใน DB จริง** (EDIEndpoints table)
...
│   │       │   ├── codeco/route.ts         # **GET CODECO outbound** (JSON/CSV/EDIFACT format)
│   │       │   ├── codeco/send/route.ts    # **POST SFTP send** — generate + upload CODECO via SFTP
│   │       │   └── endpoints/route.ts      # **GET/POST/PUT/DELETE** EDI endpoint settings (DB CRUD)
...
│       ├── schema.sql            # SQL schema reference (15 tables)
...
│
├── src/middleware.ts              # **🔐 Next.js API auth middleware** — JWT enforcement on ALL /api/ routes
└── package.json
```
...
| `EDIEndpoints` | name, shipping_line, type (sftp/ftp/api), host, port, username, password, remote_path, format, is_active, last_sent_at, last_status | **ตั้งค่า SFTP/FTP/API endpoints สำหรับส่ง EDI ให้สายเรือ** |
...
### EDI (CODECO Outbound + SFTP)

| Method | Endpoint | คำอธิบาย |
|--------|----------|---------|
| GET | `/api/edi/codeco?yard_id=X&date_from=&date_to=&type=gate_in|gate_out&shipping_line=&format=json|csv|edifact` | **CODECO outbound** — generate Container Departure/Arrival messages (UN/EDIFACT D:95B:UN, CSV, or JSON) |
| POST | `/api/edi/codeco/send` | **SFTP send** — `{ endpoint_id, yard_id, date_from, date_to, type, shipping_line }` → generate CODECO + upload via SFTP → log results |
| GET | `/api/edi/codeco/send?endpoint_id=X` | **Send log history** — ประวัติการส่งทั้งหมด |
| GET/POST/PUT/DELETE | `/api/edi/endpoints` | **EDI endpoint CRUD** — manage SFTP/FTP/API configurations stored in DB + **audit log** |
...
| `src/middleware.ts` | ทุก `/api/*` | **🔐 JWT enforcement** — ตรวจ Bearer token ทุก API call, exempt: login, EIR, uploads |
...
- **EDI Configuration**: SFTP/FTP/API endpoints — **CRUD เก็บใน DB จริง** (EDIEndpoints table) + แสดง shipping_line, remote_path, last sent status/time
...
- [x] **EDI Endpoints (DB)** — ตั้งค่า SFTP/FTP/API endpoints เก็บใน DB จริง + CRUD API + Send Log
...
- [x] **API Auth Middleware** — `src/middleware.ts` ตรวจ JWT บนทุก `/api/` route อัตโนมัติ (exempt: login, EIR, uploads)
...
| **Auth middleware** | ~~ยังไม่มี middleware ตรวจ JWT ที่ API routes~~ → **แก้แล้ว** `src/middleware.ts` ตรวจ JWT ทุก API route |
