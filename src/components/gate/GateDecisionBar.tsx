...
export default function GateDecisionBar({ signals, compact = false }: GateDecisionBarProps) {
  ...
  return (
    <section className={`${compact ? 'relative z-0' : 'sticky top-16 z-20'} rounded-xl border border-slate-200 dark:border-slate-700 bg-white/95 dark:bg-slate-800/95 backdrop-blur px-3 py-3 shadow-sm`}>
      ...
      <div className={`grid ${compact ? 'grid-cols-1' : 'grid-cols-1 sm:grid-cols-2 xl:grid-cols-5'} gap-2`}>
        {signals.items.map(item => (
          <div key={item.key} className={`h-full ${compact ? 'min-h-[62px]' : 'min-h-[72px]'} rounded-lg border px-3 py-2 ${toneClass[item.status]}`}>
            <div className="flex h-full flex-col justify-between gap-1">
              ...
              <p className="text-[10px] leading-4 opacity-80 line-clamp-1">{item.detail}</p>
            </div>
          </div>
        ))}
        <div className={`h-full ${compact ? 'min-h-[62px]' : 'min-h-[72px]'} rounded-lg border px-3 py-2 ${actionTone}`}>
          <div className="flex h-full flex-col justify-between gap-1">
            <div className="flex items-start justify-between gap-2">
              <div className="min-w-0">
                <div className="text-[10px] font-semibold uppercase tracking-normal opacity-80">สถานะงาน</div>
                <p className="mt-0.5 text-sm font-bold leading-tight">{signals.canProceed ? 'พร้อมบันทึก' : 'ขั้นตอนถัดไป'}</p>
              </div>
              <span className="mt-0.5 shrink-0">
                {signals.canProceed ? <CheckCircle2 size={14} /> : <Clock3 size={14} />}
              </span>
            </div>
            <p className="text-[10px] leading-4 opacity-80 line-clamp-1">{signals.nextAction}</p>
          </div>
        </div>
      </div>
    </section>
