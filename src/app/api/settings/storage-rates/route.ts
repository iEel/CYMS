 ...
 * Returns all tiered storage rates for a yard
 */
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ tiers: result.recordset });
  } catch (error) {
 ...
 * Save all tiers (replace all for a yard)
 * Body: { yard_id, tiers: [{ tier_name, from_day, to_day, rate_20, rate_40, rate_45, applies_to }] }
 */
export async function POST(request: NextRequest) {
  try {
    const { yard_id, tiers } = await request.json();
    ...
    // Soft-delete all existing tiers for this yard
    await db.request()
      .input('yardId', sql.Int, yard_id)
      .query('UPDATE StorageRateTiers SET is_active = 0, updated_at = GETDATE() WHERE yard_id = @yardId');
    ...
    for (let i = 0; i < tiers.length; i++) {
      ...
      await db.request()
        ...
        .query(`
          INSERT INTO StorageRateTiers (yard_id, tier_name, from_day, to_day, rate_20, rate_40, rate_45, applies_to, sort_order)
          VALUES (@yardId, @tierName, @fromDay, @toDay, @rate20, @rate40, @rate45, @appliesTo, @sortOrder)
        `);
    }
