...
export async function POST(request: NextRequest) {
  try {
    ...
    const createResult = await bindTemplateVersionConfig(createRequest, normalized.config).query(`
    ...
    `);
    const { template, version } = splitTemplateVersionRow(createResult.recordset[0] as Record<string, unknown>);
